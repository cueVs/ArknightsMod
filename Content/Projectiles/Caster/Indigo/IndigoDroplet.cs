...
public sealed class IndigoDroplet : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        Lighting.AddLight(Projectile.Center, .15f, .10f, .24f);
        if (!Main.dedServ && age % 4 == 0)
        {
            Vector2 velocity = -Projectile.velocity * .1f;
            Dust dust = Dust.NewDustPerfect(Projectile.Center, DustID.TintableDustLighted, velocity, 80, IndigoVisuals.Lilac, .6f);
            dust.noGravity = true;
        }
    }
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        IndigoVisuals.Scatter(Projectile.Center, Wisp ? 5 : 9, Wisp ? 2f : 3.2f);
        ...
        int index = Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center, Vector2.Zero,
            ModContent.ProjectileType<IndigoBurst>(), Projectile.damage, 2f, Projectile.owner, 0, excludedTarget + 1);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        IndigoVisuals.Trail(Projectile, Wisp);
        float fade = MathHelper.Clamp(Projectile.timeLeft / 15f, 0f, 1f);
        IndigoVisuals.Droplet(Projectile.Center, Wisp ? 4.5f : Projectile.ai[0] == 1f ? 10f : 7.5f,
